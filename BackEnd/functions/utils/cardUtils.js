...
async function sendUsedCardResponse(gameId, usedCard, req) {
    ...
    const userTable = markNewEquipements(updatedGame.playerTable);
    const rivalTable = markNewEquipements(updatedGame.rivalTable);
...
async function handleCreatureCard(game, usedCard, req, res) {
    ...
    return sendGameResponse(game._id, usedCard, req);
}
...
async function handleSpellCard(game, usedCard, target, req, res) {
  ...
  switch (usedCard.effect) {
    case 'protect_one': {
      ...
      return sendGameResponse(gameId, usedCard, req);
    }
